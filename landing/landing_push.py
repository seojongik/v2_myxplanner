...
"""
랜딩 페이지 - GitHub에 업로드
landing/ 폴더만 v2_landing 리포지토리로 push
"""
import subprocess
import sys
...
# 설정
REMOTE_NAME = 'landing-origin'
REMOTE_URL = 'https://github.com/seojongik/v2_landing.git'
...
def main():
    ...
    print(f"{Colors.BOLD}랜딩 페이지 - v2_landing 리포지토리로 Push{Colors.RESET}")
    print(f"대상 폴더: {FOLDER_PREFIX}/")
    print(f"대상 리포: {REMOTE_URL}")
    print()

    # 1. landing/ 폴더의 변경사항 확인
    ...
    run_command(
        ['git', 'commit', '-m', f"[Landing] {commit_message}"],
        cwd=project_root
    )
    print_success(f"커밋 완료: [Landing] {commit_message}")
    print()
    ...
    # 6. Subtree push
    response = input(f"{Colors.YELLOW}v2_landing 리포지토리에 Push하시겠습니까? (y/N): {Colors.RESET}").lower()
    if response != 'y':
