...
def run_command(cmd, cwd=None, check=True):
    try:
        ...
        if check:
            ...
            if e.stderr:
                print_error(f"에러 출력: {e.stderr}")
            if e.stdout:
                print_error(f"표준 출력: {e.stdout}")
            # stderr가 비어있을 경우 stdout을 확인
            if not e.stderr and e.stdout:
                print_error(f"에러: {e.stdout}")
            raise
        return e.stderr.strip() if e.stderr else e.stdout.strip(), e.returncode

def main():
    project_path = Path(__file__).parent

    print(f"{Colors.BOLD}골프 플래너 앱 - 로컬 변경사항 GitHub에 업로드{Colors.RESET}")
        ...
        if ahead_behind:
            ...
            if int(ahead) > 0:
                ...
                if response == 'y':
                    ...
                    run_command(['git', 'push', 'origin', current_branch], cwd=project_path)
    ...
    diff_output, _ = run_command(['git', 'diff', '--stat'], cwd=project_path, check=False)
    if diff_output:
        print(diff_output)
    print()
    ...
    staged_files, _ = run_command(['git', 'diff', '--cached', '--name-only'], 
                                  cwd=project_path, check=False)
    if not staged_files:
        print_warning("스테이징된 파일이 없습니다. 변경사항이 모두 무시되었거나 이미 커밋되었을 수 있습니다.")
        sys.exit(0)
