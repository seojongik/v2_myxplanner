...
class TimePassSearchPage extends StatefulWidget {
  final bool isAdminMode;
  final Map<String, dynamic>? selectedMember;
  final String? branchId;

  const TimePassSearchPage({
    Key? key,
  ...
  @override
  _TimePassSearchPageState createState() => _TimePassSearchPageState();
}

class _TimePassSearchPageState extends State<TimePassSearchPage> {
  @override
...
class TimePassSearchContent extends StatefulWidget {
  final bool isAdminMode;
  final Map<String, dynamic>? selectedMember;
  final String? branchId;

  const TimePassSearchContent({
    Key? key,
    this.isAdminMode = false,
    this.selectedMember,
    this.branchId,
  ...
  @override
  _TimePassSearchContentState createState() => _TimePassSearchContentState();
}

class _TimePassSearchContentState extends State<TimePassSearchContent> {
  bool _isLoading = true;
  bool _showExpired = false;
  List<Map<String, dynamic>> _contracts = [];
  Map<String, dynamic>? _selectedContract;
  List<Map<String, dynamic>> _timePassHistory = [];
  ...
  Future<void> _loadContracts() async {
    ...
    try {
      ...
      print('=== 시간권 조회 디버깅 시작 (v2_bill_times 중심) ===');
      ...
      // 1. v2_bill_times에서 회원의 모든 시간권 거래 내역 조회
      print('\n=== STEP 1: v2_bill_times에서 모든 시간권 거래 조회 ===');
      ...
      print('v2_bill_times에서 조회된 전체 거래 수: ${allBills.length}');
      ...
      for (final entry in contractMap.entries) {
        ...
        print('최신 거래 정보:');
        print('  - bill_min_id: ${latestBill['bill_min_id']}');
        print('  - bill_balance_min_after: ${balance}분');
        print('  - contract_TS_min_expiry_date: $expiryDateStr');
        ...
        contractList.add({
          ...
          'contract_TS_min_expiry_date': expiryDateStr,
          'last_balance': balance.toString(),
          'bill_count': billCount,
          'is_valid': isValid,
        });

        print('✅ 계약 추가: contract_history_id=$contractHistoryId, 잔액: ${balance}분, 거래: $billCount건');
      }
      ...
      _contracts.sort((a, b) {
        try {
          final dateA = DateTime.parse(b['contract_TS_min_expiry_date']);
          final dateB = DateTime.parse(a['contract_TS_min_expiry_date']);
      ...
      for (int i = 0; i < _contracts.length; i++) {
        ...
        print('  $i: contract_history_id=${c['contract_history_id']}, ${c['bill_text']}, 잔액: ${c['last_balance']}분, 만료: ${c['contract_TS_min_expiry_date']}, 유효: ${c['is_valid']}');
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('시간권 정보를 불러오는데 실패했습니다: $e'),
  ...
  Future<void> _loadTimePassHistory(String contractHistoryId) async {
    setState(() => _isLoading = true);
    ...
    try {
      ...
      _timePassHistory = await ApiService.getData(
        table: 'v2_bill_times',
        where: [
          {'field': 'branch_id', 'operator': '=', 'value': branchId},
          {'field': 'member_id', 'operator': '=', 'value': memberId},
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('시간권 내역을 불러오는데 실패했습니다: $e')),
        );
      }
    } finally {
      if (mounted) setState(() => _isLoading = false);
  ...
  bool _hasBalanceError(Map<String, dynamic> history) {
    final billMin = int.tryParse(history['bill_min']?.toString() ?? '0') ?? 0;
    ...
    // 잔액변화량의 절대값과 bill_min이 같아야 정상
    return calculatedChange.abs() != billMin;
  }
  ...
  Widget _buildContractTile(Map<String, dynamic> contract) {
    ...
    final expiryDate = _formatDate(contract['contract_TS_min_expiry_date']);
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: () {
            ...
            _loadTimePassHistory(contract['contract_history_id']);
          },
          child: Container(
            padding: EdgeInsets.all(16),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(12),
              border: Border.all(
                color: isValid ? Color(0xFF9C27B0).withOpacity(0.2) : Color(0xFFE0E0E0),
                width: 1,
              ),
            ),
            child: Column(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 4),
                          Text(
                            _formatTime(contract['last_balance']),
                            style: TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.bold,
                              color: isValid ? Color(0xFF9C27B0) : Color(0xFF999999),
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              IconButton(
                icon: Icon(Icons.arrow_back, color: Color(0xFF9C27B0)),
                onPressed: () {
                  setState(() {
                    _selectedContract = null;
                    _timePassHistory.clear();
                  });
                },
              ),
              SizedBox(width: 8),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '만료일: ${_formatDate(_selectedContract?['contract_TS_min_expiry_date'])}',
                      style: TextStyle(
                        fontSize: 12,
                        color: Color(0xFF666666),
                      ),
        ...
        Expanded(
          child: _timePassHistory.isEmpty
              ? Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
              ...
              : ListView.builder(
                  padding: EdgeInsets.all(16),
                  itemCount: _timePassHistory.length,
                  itemBuilder: (context, index) {
                    final history = _timePassHistory[index];
  ...
  Widget _buildHistoryTile(Map<String, dynamic> history) {
    ...
    final balanceAfter = history['bill_balance_min_after'];
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    '${isCredit ? '+' : ''}${_formatTime(actualChange)}',
                    style: TextStyle(
                      fontSize: 16,
                      fontWeight: FontWeight.bold,
                      color: hasError 
