...
import 'package:famd_clientapp/services/api_service.dart'; // 새로운 API 서비스 import
...
class _MenuScreenState extends State<MenuScreen> {
  ...
  Future<List<Map<String, dynamic>>> _fetchAllTodayReservations() async {
    ...
    try {
      // 1. 일반 예약 가져오기 - dynamic_api.php 사용
      ...
      final lessonStatusResponse = await _fetchLessonStatusFromAPI(memberId);
  ...
  // API를 사용하여 레슨 상태 정보 가져오기 - dynamic_api.php 사용
  Future<Map<String, dynamic>> _fetchLessonStatusFromAPI(dynamic memberId) async {
    try {
      // 1. 레슨 카운팅 정보 조회
      ...
      if (countingsResponse.statusCode == 200) {
        final countingsResp = jsonDecode(countingsResponse.body);
        if (countingsResp['success'] == true && countingsResp['data'] != null) {
          countings = countingsResp['data'];
        }
      }
      
      // 주문 데이터 처리
      if (ordersResponse.statusCode == 200) {
        final ordersResp = jsonDecode(ordersResponse.body);
  ...
  // 프로 정보 조회 함수 (dynamic_api.php 사용)
  Future<String> _getProNameById(int proId) async {
    try {
