...
import 'package:famd_clientapp/screens/subpages/pages/junior_reservation_info.dart'; // 주니어 예약 정보 화면 import
import 'package:famd_clientapp/services/junior_lesson_cancellation_service.dart'; // 주니어 레슨 취소 서비스 import
import 'package:famd_clientapp/services/api_service.dart'; // 새로운 ApiService import
import 'package:famd_clientapp/services/reservation_service.dart'; // 새로운 ReservationService import
...
class _MenuScreenState extends State<MenuScreen> {
  ...
  // 정보 행 위젯 (lesson_reservation_history.dart와 유사한 방식)
  ...
  Future<List<Map<String, dynamic>>> _fetchAllTodayReservations() async {
    ...
    try {
      // 1. 일반 예약 가져오기 (ReservationService 사용)
      final reservations = await ReservationService.getTSReservationsByMember(int.tryParse(memberId ?? '') ?? 0, branchId);
      final todayList = reservations.where((r) => r['ts_date'] == todayStr).toList();
      
      // 일반 예약임을 표시
      for (var reservation in todayList) {
        reservation['isJunior'] = false;
        reservation['isTSReservation'] = true;
        allReservations.add(reservation);
      }
      
      // 2. 레슨 예약 내역 가져오기 (lesson_reservation_history.dart와 동일한 방식)
      final lessonStatusResponse = await _fetchLessonStatusFromAPI(memberId, branchId);
      ...
      if (lessonStatusResponse.containsKey('orders') && lessonStatusResponse['orders'] is List) {
        ...
        if (orders.isNotEmpty) {
          ...
          for (var lesson in todayLessons) {
            lesson['isJunior'] = false;
            lesson['isLessonReservation'] = true;
            allReservations.add(lesson);
          }
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    final memberId = userProvider.user?.id;
    final todayStr = DateFormat('yyyy-MM-dd').format(DateTime.now());
    Map<String, dynamic>? todayReservation;
    ...
    // 오늘 예약을 불러오는 FutureBuilder
    Widget todayReservationTile() {
                          ...
                          builder: (context) {
                            return Padding(
                              ...
                              child: ReservationDetailModal(
                                reservation: r,
                                memberId: int.tryParse(memberId.toString()),
  ...
  void _showJuniorReservationDetail(BuildContext context, Map<String, dynamic> reservation) {
    ...
    Future<void> performCancel(String lsSetId) async {
      ...
      try {
        // 예약 취소 서비스 호출
        final result = await ApiService.cancelLessonReservation(reservation['id']);
        ...
        if (result['success']) {
          ...
          // 안전하게 Future를 사용하여 지연 후 공지사항 로드
