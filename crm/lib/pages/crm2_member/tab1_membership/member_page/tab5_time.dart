...
class Tab5TimeWidget extends StatefulWidget {
  const Tab5TimeWidget({
    super.key,
  ...
  @override
  State<Tab5TimeWidget> createState() => _Tab5TimeWidgetState();
}

class _Tab5TimeWidgetState extends State<Tab5TimeWidget> {
  List<Map<String, dynamic>> timeHistory = [];
  ...
  Future<void> _loadContractsAndBalances() async {
    ...
    try {
      // 1. 해당 회원의 모든 bill_times 데이터 조회하여 contract_history_id별로 그룹핑
      final allTimeData = await ApiService.getBillTimesData(
        where: [
          {
            'field': 'member_id',
            'operator': '=',
      ...
      for (final time in allTimeData) {
        final contractHistoryId = time['contract_history_id'];
        ...
        if (!contractGroups.containsKey(contractHistoryId)) {
          ...
          String contractName = '시간권 #$contractHistoryId';
          try {
          ...
          contractGroups[contractHistoryId] = {
            ...
            'current_balance': int.tryParse(time['bill_balance_min_after']?.toString() ?? '0') ?? 0,
            'last_transaction': time,
            'expiry_date': time['contract_TS_min_expiry_date'],
          };
        }
      }
  ...
  void _applyFilters() {
    ...
    if (filteredContractsWithBalance.isNotEmpty &&
      ...
      _loadTimeHistory(selectedContract!['contract_history_id']);
  ...
  // 선택된 계약의 시간권 내역 조회
  Future<void> _loadTimeHistory(int? contractHistoryId) async {
  ...
  Color _getBillTypeColor(String? billType) {
    ...
    if (billType.contains('등록') || billType.contains('적립')) {
      return Colors.blue;
    } else if (billType.contains('사용') || billType.contains('차감')) {
      return Colors.red;
    }
    return Colors.black;
  }
  ...
  // bill_min 계산 (등록은 +, 사용은 -)
  String _formatBillMin(dynamic billMin, String? billType) {
    if (billMin == null) return '0분';
    
    try {
      final intBillMin = int.parse(billMin.toString());
      if (billType?.contains('사용') == true || billType?.contains('차감') == true) {
        return '-${_formatMinutes(intBillMin)}';
      } else {
        return '+${_formatMinutes(intBillMin)}';
      }
    } catch (e) {
                          ...
                          return GestureDetector(
                            onTap: () {
                              setState(() {
                                ...
                                _loadTimeHistory(contract['contract_history_id']);
                              });
                            },
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (contract['expiry_date'] != null)
                                    Text(
                                      '유효기간: ${_formatDate(contract['expiry_date'])}',
                                      style: AppTextStyles.caption.copyWith(
                                        fontFamily: 'Pretendard',
                                        color: Color(0xFF94A3B8),
                                        fontSize: 11,
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text(
                          selectedContract?['contract_name'] ?? '시간권',
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: ListView.builder(
                                          ...
                                          itemBuilder: (context, index) {
                                            final item = timeHistory[index];
                                            final billType = item['bill_type'];
                                            final isUsage = billType == '타석이용' || billType?.contains('차감') == true;
                                              ...
                                              child: Row(
                                                children: [
                                                  ...
                                                  Expanded(
                                                    flex: 1,
                                                    child: Text(
                                                      _formatMinutes(item['bill_balance_min_before']),
                                                      style: AppTextStyles.cardSubtitle.copyWith(
                                                        fontFamily: 'Pretendard',
                                                        color: Color(0xFF6B7280),
                                                        fontWeight: FontWeight.w600,
                                                  ...
                                                  Expanded(
                                                    flex: 1,
                                                    child: Text(
                                                      _formatBillMin(item['bill_min'], billType),
                                                      style: AppTextStyles.cardSubtitle.copyWith(
                                                        fontFamily: 'Pretendard',
                                                  ...
                                                  Expanded(
                                                    flex: 1,
                                                    child: Text(
                                                      _formatMinutes(item['bill_balance_min_after']),
                                                      style: AppTextStyles.cardSubtitle.copyWith(
                                                        fontFamily: 'Pretendard',
                                                        color: Color(0xFF1E293B),
                                                        fontWeight: FontWeight.w600,
