...
class Tab2CustomerBoardWidget extends StatefulWidget {
  @override
  _Tab2CustomerBoardWidgetState createState() => _Tab2CustomerBoardWidgetState();
}

class _Tab2CustomerBoardWidgetState extends State<Tab2CustomerBoardWidget> {
  ...
  Future<void> _loadBoardData() async {
    print('🔍 [UI DEBUG] _loadBoardData 시작 - 고객게시판');
    ...
    try {
      final whereConditions = [
        {'field': 'board_type', 'operator': 'IN', 'value': ['자유게시판', '라운딩 모집', '중고판매']},
      ...
      print('✅ [UI DEBUG] 고객게시판 데이터 로드 성공: ${data.length}개');
      setState(() {
        boardList = data;
      });
    } catch (e) {
      print('❌ [UI DEBUG] 고객게시판 데이터 로드 실패: $e');
...
class _CreatePostDialogState extends State<_CreatePostDialog> {
  ...
  Future<void> _createPost() async {
    print('🔍 [UI DEBUG] _createPost 시작 - 고객게시판');
    print('📝 [UI DEBUG] 제목: "${_titleController.text.trim()}"');
    print('📝 [UI DEBUG] 내용: "${_contentController.text.trim()}"');
    print('🏷️ [UI DEBUG] 선택된 board_type: $_selectedBoardType');
    print('📊 [UI DEBUG] 선택된 post_status: $_selectedPostStatus');
    print('📅 [UI DEBUG] 선택된 due_date: $_selectedDueDate');
    ...
    try {
      ...
      // post_status는 라운딩 모집, 중고판매에서만 추가
      if (_selectedBoardType == '라운딩 모집' || _selectedBoardType == '중고판매') {
        postData['post_status'] = _selectedPostStatus;
      }
      
      // post_due_date는 라운딩 모집에서만 추가 (선택된 경우)
      if (_selectedBoardType == '라운딩 모집' && _selectedDueDate != null) {
        postData['post_due_date'] = '${_selectedDueDate!.year}-${_selectedDueDate!.month.toString().padLeft(2, '0')}-${_selectedDueDate!.day.toString().padLeft(2, '0')}';
      }
      ...
      _showSuccessSnackBar('게시글이 작성되었습니다');
    } catch (e) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: ['자유게시판', '라운딩 모집', '중고판매'].map((String value) {
                    ...
                    // post_status 필드 (라운딩 모집, 중고판매에서만 표시)
                    if (_selectedBoardType == '라운딩 모집' || _selectedBoardType == '중고판매') ...[
                      SizedBox(height: 16),
                      Text(
                        '상태',
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: (String? newValue) {
                          setState(() {
                            _selectedPostStatus = newValue!;
                          });
                        },
                      ),
                    ],
                    
                    // post_due_date 필드 (라운딩 모집에서만 표시)
                    if (_selectedBoardType == '라운딩 모집') ...[
                      SizedBox(height: 16),
                      Text(
                        '모집 마감일 (선택사항)',
                        style: TextStyle(
                            ...
                            children: [
                              ...
                              Text(
                                _selectedDueDate != null 
                                  ...
                                  : '마감일 선택',
                                style: TextStyle(
                                  color: _selectedDueDate != null ? Colors.black : Colors.grey[500],
                                  fontSize: 14,
                                ),
                              ),
                              Spacer(),
...
class _PostDetailDialogState extends State<_PostDetailDialog> {
  ...
  void _showEditPostDialog() {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return _EditCustomerPostDialog(
          post: widget.post,
  ...
  Future<void> _deletePost() async {
    print('🔍 [UI DEBUG] _deletePost 시작 - 고객게시판 (DetailDialog)');
    ...
    try {
      ...
      _showSuccessSnackBar('게시글이 삭제되었습니다');
...
class _EditCustomerPostDialog extends StatefulWidget {
  final Map<String, dynamic> post;
  final VoidCallback onPostUpdated;

  const _EditCustomerPostDialog({
    Key? key,
    required this.post,
    required this.onPostUpdated,
  }) : super(key: key);
  ...
  @override
  _EditCustomerPostDialogState createState() => _EditCustomerPostDialogState();
}

class _EditCustomerPostDialogState extends State<_EditCustomerPostDialog> {
  late final TextEditingController _titleController;
  late final TextEditingController _contentController;
  late String _selectedBoardType;
  late String _selectedPostStatus;
  DateTime? _selectedDueDate;
  ...
  void initState() {
    ...
    _selectedBoardType = widget.post["board_type"] ?? "자유게시판";
    _selectedPostStatus = widget.post["post_status"] ?? "진행";
    
    // 마감일 파싱
    if (widget.post["post_due_date"] != null && widget.post["post_due_date"].toString().isNotEmpty) {
      try {
        _selectedDueDate = DateTime.parse(widget.post["post_due_date"]);
      } catch (e) {
        print("날짜 파싱 실패: ${widget.post["post_due_date"]}");
        _selectedDueDate = null;
      }
    }
  }
  ...
  Future<void> _updatePost() async {
    ...
    try {
      ...
      // post_status는 라운딩 모집, 중고판매에서만 추가
      if (_selectedBoardType == "라운딩 모집" || _selectedBoardType == "중고판매") {
        updateData["post_status"] = _selectedPostStatus;
      } else {
        updateData["post_status"] = null;
      }
      
      // post_due_date는 라운딩 모집에서만 추가 (선택된 경우)
      if (_selectedBoardType == "라운딩 모집" && _selectedDueDate != null) {
        updateData["post_due_date"] = "${_selectedDueDate!.year}-${_selectedDueDate!.month.toString().padLeft(2, "0")}-${_selectedDueDate!.day.toString().padLeft(2, "0")}";
      } else {
        updateData["post_due_date"] = null;
      }
      ...
      Navigator.of(context).pop();
      _showSuccessSnackBar("게시글이 수정되었습니다");
            ...
            Flexible(
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    // 게시글 유형
                    Text("게시글 유형", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
                    SizedBox(height: 8),
                    DropdownButtonFormField<String>(
                      value: _selectedBoardType,
                      decoration: InputDecoration(
                        filled: true, fillColor: Colors.white,
                        enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.grey[300]!, width: 1), borderRadius: BorderRadius.circular(8)),
                        focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Color(0xFF10B981), width: 2), borderRadius: BorderRadius.circular(8)),
                        border: OutlineInputBorder(borderSide: BorderSide(color: Colors.grey[300]!, width: 1), borderRadius: BorderRadius.circular(8)),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                      ),
                      dropdownColor: Colors.white,
                      items: ["자유게시판", "라운딩 모집", "중고판매"].map((String value) {
                        return DropdownMenuItem<String>(value: value, child: Text(value, style: TextStyle(color: Colors.black, fontSize: 14)));
                      }).toList(),
                      onChanged: (String? newValue) {
                        setState(() {
                          _selectedBoardType = newValue!;
                    ...
                    // 제목
                    Text("제목", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
                    SizedBox(height: 8),
                    TextField(
                      controller: _titleController,
                      style: TextStyle(color: Colors.black, fontSize: 14),
                      decoration: InputDecoration(
                        hintText: "게시글 제목을 입력하세요", hintStyle: TextStyle(color: Colors.grey[500], fontSize: 14),
                        enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.grey[300]!, width: 1), borderRadius: BorderRadius.circular(8)),
                        focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Color(0xFF10B981), width: 2), borderRadius: BorderRadius.circular(8)),
                        border: OutlineInputBorder(borderSide: BorderSide(color: Colors.grey[300]!, width: 1), borderRadius: BorderRadius.circular(8)),
                        contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                    ...
                    // 내용
                    Text("내용", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
                    SizedBox(height: 8),
                    Container(
                      height: 120,
                      child: TextField(
                        ...
                        style: TextStyle(color: Colors.black, fontSize: 14),
                        decoration: InputDecoration(
                          hintText: "게시글 내용을 입력하세요", hintStyle: TextStyle(color: Colors.grey[500], fontSize: 14),
                          enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: Colors.grey[300]!, width: 1), borderRadius: BorderRadius.circular(8)),
                          focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Color(0xFF10B981), width: 2), borderRadius: BorderRadius.circular(8)),
                          border: OutlineInputBorder(borderSide: BorderSide(color: Colors.grey[300]!, width: 1), borderRadius: BorderRadius.circular(8)),
                          contentPadding: EdgeInsets.all(12),
                    ...
                    // 상태 필드 (라운딩 모집, 중고판매에서만)
                    if (_selectedBoardType == "라운딩 모집" || _selectedBoardType == "중고판매") ...[
                      SizedBox(height: 16),
                      Text("상태", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
                      SizedBox(height: 8),
                      DropdownButtonFormField<String>(
                        value: _selectedPostStatus,
                        decoration: InputDecoration(
                    ...
                    if (_selectedBoardType == "라운딩 모집") ...[
                      SizedBox(height: 16),
                      Text("모집 마감일 (선택사항)", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
                      SizedBox(height: 8),
                      InkWell(
                        onTap: () async {
                          final DateTime? picked = await showDatePicker(
